...
public class Client {
    ...
    public RpcReference initClientApplication() throws IOException, ClassNotFoundException, InstantiationException, IllegalAccessException {
        ...
        // 设置Bootstrap对象的EventLoopGroup和通道类型
        bootstrap.group(clientGroup);
        bootstrap.channel(NioSocketChannel.class);
        // 设置了一个ChannelInitializer，用于初始化SocketChannel的ChannelPipeline。
        bootstrap.handler(new ChannelInitializer<SocketChannel>() {
            @Override
            protected void initChannel(SocketChannel ch) throws Exception {
                // 在initChannel方法中，添加了一系列的处理器
        ...
        // 从本地加载客户端配置，并将其保存到clientConfig对象中
        this.clientConfig = PropertiesBootstrap.loadClientConfigFromLocal();
        CLIENT_CONFIG = this.clientConfig;
        this.initClientConfig();
        // 使用扩展加载器（EXTENSION_LOADER）加载ProxyFactory接口的实现类，
        EXTENSION_LOADER.loadExtension(ProxyFactory.class);
        // 并根据客户端配置中指定的代理类型获取相应的代理工厂类。
